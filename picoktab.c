...
static pico_status_t
ktabIdsSubObjDeallocate(register picoknow_KnowledgeBase this,
...
picoktab_FixedIds picoktab_newFixedIds(picoos_MemoryManager mm) {
  picoktab_FixedIds this =
      (picoktab_FixedIds)picoos_allocate(mm, sizeof(*this));
  if (NULL != this) {
    /* initialize */
  }
  return this;
...
void picoktab_disposeFixedIds(picoos_MemoryManager mm,
                              picoktab_FixedIds *this) {
  if (NULL != (*this)) {
    /* terminate */
...
/* overview binary file format for graphs kb:
    ...
    OFFSET: SIZEOFSENTRY bytes, offset to baseaddress of graphs-kb to entry in
   graphs

    graphs = {graph}=NROFSENTRIES (contains NROFSENTRIES entries of graph)

    graph = PROPSET FROM TO [TOKENTYPE] [TOKENSUBTYPE] [VALUE] [LOWERCASE]
   [GRAPHSUBS1] [GRAPHSUBS2] [PUNCT]
    ...
    PROPSET       : 1 unsigned byte, least significant bit : has TO field
                                                             next bit : has
   TOKENTYPE next bit : has TOKENSUBTYPE next bit : has VALUE next bit : has
   LOWERCASE next bit : has GRAPHSUBS1 next bit : has GRAPHSUBS2 next bit : has
   PUNC
...
static pico_status_t ktabGraphsInitialize(register picoknow_KnowledgeBase this,
  ...
  ktabgraphs->nrOffset =
      ((int)(this->base[KTAB_START_GRAPHS_NR_OFFSET])) +
...
static pico_status_t
ktabGraphsSubObjDeallocate(register picoknow_KnowledgeBase this,
                           picoos_MemoryManager mm) {
  if (NULL != this) {
    picoos_deallocate(mm, (void *)&this->subObj);
  }
...
pico_status_t
picoktab_specializeGraphsKnowledgeBase(picoknow_KnowledgeBase this,
                                       picoos_Common common) {
  if (NULL == this) {
    return picoos_emRaiseException(common->em, PICO_EXC_KB_MISSING, NULL, NULL);
  }
...
picoos_uint8 picoktab_hasVowellikeProp(const picoktab_Graphs this,
  ...
  return g->graphTable[graphsOffset +
                       ktab_propOffset(this, graphsOffset,
...
static void ktab_getStrProp(const picoktab_Graphs this,
                            picoos_uint32 graphsOffset,
                            picoos_uint32 propOffset, picoos_uchar *str) {
  ktabgraphs_subobj_t *g = (ktabgraphs_SubObj)this;
  picoos_uint32 i, l;
...
static picoos_uint32 ktab_propOffset(const picoktab_Graphs this,
                                     picoos_uint32 graphsOffset,
                                     picoos_uint32 prop)
/* Returns offset of property 'prop' inside the graph with offset 'graphsOffset'
 in graphs table; If the property is found, a value > 0 is returned otherwise 0
 */
{
  picoos_uint32 n = 0;
  ktabgraphs_subobj_t *g = (ktabgraphs_SubObj)this;

  // Check if the propset at the given offset includes the given property,
  // otherwise return 0
  if ((g->graphTable[graphsOffset] & prop) == prop) {

    n = n + 1; /* overread PROPSET field */
    n = n + picobase_det_utf8_length(
                g->graphTable[graphsOffset + n]); /* overread FROM field */
    ...
    if (prop > KTAB_GRAPH_PROPSET_GRAPHSUBS1) {
      if ((g->graphTable[graphsOffset] & KTAB_GRAPH_PROPSET_GRAPHSUBS1) ==
        ...
        n = n + picobase_det_utf8_length(
                    g->graphTable[graphsOffset +
                                  n]); /* overread graphsubs1 field */
      }
    } else {
      return n;
    }
...
picoos_uint32 picoktab_graphOffset(const picoktab_Graphs this,
  ...
  if (g->nrOffset > 0) {
    ...
    do {
      ...
      /* PICODBG_DEBUG(("picoktab_graphOffset: %i %i %i '%s' '%s' '%s'", a, m,
       * b, from, utf8graph, to));
       */
      utfGEfrom =
          picoos_strcmp((picoos_char *)utf8graph, (picoos_char *)from) >= 0;
      utfLEto = picoos_strcmp((picoos_char *)utf8graph, (picoos_char *)to) <= 0;

      if (utfGEfrom && utfLEto) {
        /* PICODBG_DEBUG(("picoktab_graphOffset: utf char '%s' found",
         * utf8graph));
         */
        return graphsOffset;
      }
...
picoos_bool picoktab_getIntPropTokenSubType(const picoktab_Graphs this,
  ...
  propOffset =
      ktab_propOffset(this, graphsOffset, KTAB_GRAPH_PROPSET_TOKENSUBTYPE);
  if (propOffset > 0) {
...
picoos_bool picoktab_getStrPropLowercase(const picoktab_Graphs this,
  ...
  propOffset =
      ktab_propOffset(this, graphsOffset, KTAB_GRAPH_PROPSET_LOWERCASE);
  if (propOffset > 0) {
...
picoos_bool picoktab_getStrPropGraphsubs1(const picoktab_Graphs this,
  ...
  propOffset =
      ktab_propOffset(this, graphsOffset, KTAB_GRAPH_PROPSET_GRAPHSUBS1);
  if (propOffset > 0) {
...
picoos_bool picoktab_getStrPropGraphsubs2(const picoktab_Graphs this,
  ...
  propOffset =
      ktab_propOffset(this, graphsOffset, KTAB_GRAPH_PROPSET_GRAPHSUBS2);
  if (propOffset > 0) {
...
void picoktab_graphsGetGraphInfo(
    const picoktab_Graphs this, picoos_uint16 graphIndex, picoos_uchar *from,
    picoos_uchar *to, picoos_uint8 *propset, picoos_uint8 *stokenType,
    picoos_uint8 *stokenSubType, picoos_uint8 *value, picoos_uchar *lowercase,
    picoos_uchar *graphsubs1, picoos_uchar *graphsubs2, picoos_uint8 *punct) {
  ktabgraphs_subobj_t *g = (ktabgraphs_SubObj)this;
  picoos_uint32 graphsOffset;
  picoos_uint8 *pos;
...
static pico_status_t
ktabPhonesSubObjDeallocate(register picoknow_KnowledgeBase this,
                           picoos_MemoryManager mm) {
  if (NULL != this) {
    picoos_deallocate(mm, (void *)&this->subObj);
  }
...
pico_status_t
picoktab_specializePhonesKnowledgeBase(picoknow_KnowledgeBase this,
                                       picoos_Common common) {
  if (NULL == this) {
    return picoos_emRaiseException(common->em, PICO_EXC_KB_MISSING, NULL, NULL);
  }
...
static pico_status_t ktabPosInitialize(register picoknow_KnowledgeBase this,
  ...
  for (i = 0, pos = 0; i < PICOKTAB_MAXNRPOS_IN_COMB; i++, pos += 4) {
    ktabpos->nrcomb[i] =
        ((picoos_uint16)(this->base[pos + 1])) << 8 | this->base[pos];
    if (ktabpos->nrcomb[i] > 0) {
      ...
      if (osprev >= os) {
        ...
        return picoos_emRaiseException(common->em, PICO_EXC_FILE_CORRUPT, NULL,
                                       NULL);
      }
    } else {
      if (i == 0) {
...
static pico_status_t
ktabPosSubObjDeallocate(register picoknow_KnowledgeBase this,
                        picoos_MemoryManager mm) {
  if (NULL != this) {
    picoos_deallocate(mm, (void *)&this->subObj);
  }
  ...
  if ((poslistlen > 0) && (poslistlen <= PICOKTAB_MAXNRPOS_IN_COMB)) {
    ...
    if (i > 0) {
      ...
      if (!poscomb) {
        ...
        PICODBG_WARN(("dynamically created POS combination not found in table; "
                      "taking first (%i)",
                      poslist[0]));
        poscomb = poslist[0];
      }
